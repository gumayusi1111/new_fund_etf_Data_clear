...
负责VMA系统的文件操作，包括目录管理、文件路径处理、批量操作等
"""
...
class VMAFileManager:
    ...
    def save_vma_result(self, etf_code: str, threshold: str,
        ...
        try:
            ...
            self.logger.debug(f"VMA结果保存成功: {output_path}")
            return True

        except Exception as e:
            self.logger.error(f"保存VMA结果失败 {etf_code}: {str(e)}")
            return False

    def load_vma_result(self, etf_code: str, threshold: str) -> Optional[pd.DataFrame]:
        """
        加载已有的VMA计算结果
        ...
        try:
            ...
            self.logger.debug(f"VMA结果加载成功: {etf_code}")
            return df

        except Exception as e:
            self.logger.error(f"加载VMA结果失败 {etf_code}: {str(e)}")
            return None
        ...
        try:
            ...
            for csv_file in data_dir.glob("*.csv"):
                try:
                    ...
                    # 检查是否已有VMA结果
                    output_path = self.config.get_output_path(threshold, etf_code)
                    file_info['has_vma_result'] = output_path.exists()
