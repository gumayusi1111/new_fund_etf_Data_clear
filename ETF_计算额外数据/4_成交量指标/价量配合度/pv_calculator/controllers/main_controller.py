...
class PVController:
    ...
    def __init__(self):
        ...
        # 初始化各个组件
        self.engine = PVEngine(self.config)
        self.data_reader = PVDataReader(self.config)
        self.cache_manager = PVCacheManager(self.config)
        self.file_manager = PVFileManager(self.config)
        self.result_processor = PVResultProcessor(self.config)

        # 初始化处理器
        self.batch_processor = PVBatchProcessor(
            self.config, self.engine, self.data_reader,
            self.cache_manager, self.file_manager, self.result_processor
        )
        self.etf_processor = PVETFProcessor(
            self.config, self.engine, self.data_reader,
            self.cache_manager, self.file_manager, self.result_processor
        )
    ...
    def calculate_single_etf(self, etf_code: str, threshold: str,
        ...
        """
        try:
            self.logger.info(f"开始计算单个ETF PV指标: {etf_code} ({threshold})")
            ...
            if result['success']:
                self.logger.info(f"ETF {etf_code} PV计算成功")
            else:
                self.logger.error(f"ETF {etf_code} PV计算失败: {result.get('error', '未知错误')}")
    ...
    def calculate_batch_etfs(self, threshold: str, etf_codes: Optional[List[str]] = None,
        ...
        """
        try:
            self.logger.info(f"开始批量计算PV指标: {threshold}")
    ...
    def test_system(self, sample_size: int = 5) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"开始PV系统测试 (样本数: {sample_size})")
            ...
            self.logger.info(f"PV系统测试完成: {status}")
    ...
    def _setup_logging(self) -> logging.Logger:
        """设置日志系统"""
        logger = logging.getLogger('PVController')
    ...
    def _test_config(self) -> Dict[str, Any]:
        ...
        try:
            # 检查关键配置
            assert len(self.config.correlation_periods) > 0
            assert len(self.config.vpt_periods) > 0
            assert self.config.volume_quality_window > 0

            return {
                'passed': True,
                'message': '配置测试通过',
                'details': {
                    'correlation_periods_count': len(self.config.correlation_periods),
                    'vpt_periods_count': len(self.config.vpt_periods)
                }
            }
        except Exception as e:
            return {
    ...
    def _test_engine(self, sample_size: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 测试PV计算
            result = self.engine.calculate_pv_indicators(data)
