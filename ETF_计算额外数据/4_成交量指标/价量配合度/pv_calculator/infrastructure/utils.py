...
class PVUtils:
    ...
    @staticmethod
    def calculate_price_volume_correlation(price_series: pd.Series, 
        ...
        try:
            ...
            logging.getLogger(__name__).error(f"计算价量相关性失败: {str(e)}")
            return pd.Series([np.nan] * len(price_series), index=price_series.index)

    @staticmethod
    def detect_price_volume_divergence(price_series: pd.Series,
                                     volume_series: pd.Series,
        ...
        try:
            ...
            # 初始化结果序列
            divergence = pd.Series(0, index=price_series.index)
            ...
            for i in range(1, len(price_change)):
                ...
                # 价涨量跌 - 负背离
                if price_change.iloc[i] > threshold and volume_change.iloc[i] < -threshold:
                    divergence.iloc[i] = -1
                # 价跌量涨 - 正背离  
                elif price_change.iloc[i] < -threshold and volume_change.iloc[i] > threshold:
                    divergence.iloc[i] = 1
                    
            return divergence
            
        except Exception as e:
            logging.getLogger(__name__).error(f"检测价量背离失败: {str(e)}")
            return pd.Series(0, index=price_series.index)

    @staticmethod
    def calculate_volume_price_trend(price_series: pd.Series,
                                   volume_series: pd.Series,
        ...
        try:
            ...
            # 计算价格趋势(相对于均线)
            price_trend = (price_series - price_ma) / price_ma
            
            # 计算成交量趋势(相对于均线)
            volume_trend = (volume_series - volume_ma) / volume_ma
            
            # 计算量价趋势综合指标
            vpt = price_trend * volume_trend
            
            return vpt
            
        except Exception as e:
            logging.getLogger(__name__).error(f"计算量价趋势失败: {str(e)}")
            return pd.Series([np.nan] * len(price_series), index=price_series.index)