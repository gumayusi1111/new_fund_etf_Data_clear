...
class MomentumCacheManager:
    ...
    def __init__(self, cache_dir: Optional[Path] = None, meta_dir: Optional[Path] = None, 
                 expire_days: Optional[int] = None, max_size_mb: Optional[int] = None):
        """
        初始化缓存管理器
        
        Args:
            cache_dir: 缓存目录，None则使用配置默认值
            meta_dir: 元数据目录，None则使用配置默认值
            expire_days: 缓存过期天数，None则使用配置默认值
            max_size_mb: 最大缓存大小(MB)，None则使用配置默认值
        """
        config = MomentumConfig.CACHE_CONFIG
        
        self.cache_dir = cache_dir or MomentumConfig.CACHE_DIR
        self.meta_dir = meta_dir or MomentumConfig.CACHE_META_DIR
        self.expire_days = expire_days or config['expire_days']
        ...
        self.logger.info(f"缓存管理器初始化完成 - 目录:{self.cache_dir}, 过期:{self.expire_days}天")
    ...
    def get_cache_key(self, etf_code: str, threshold: str) -> str:
        ...
        threshold_key = threshold_map.get(threshold, threshold)
        return f"{etf_code}_{threshold_key}"
    ...
    def get_cache_path(self, etf_code: str, threshold: str) -> Path:
        ...
        """
        # 使用配置中定义的门槛缓存目录
        cache_subdir = MomentumConfig.get_cache_dir(threshold)
    ...
    def load_metadata(self, etf_code: str, threshold: str) -> Optional[MomentumCacheMetadata]:
        """
        ...
        try:
            ...
            with open(meta_path, 'r', encoding='utf-8') as f:
                ...
                # 从阈值级别聚合数据中获取特定ETF信息
                if etf_code not in threshold_meta.get('processing_stats', {}):
                    return None
                
                # 查找ETF数据（可能在system_info或其他位置）
                etf_meta = None
                if 'fields_description' in threshold_meta:
                    # 这是新格式的meta文件，查找缓存信息
                    cache_path = self.get_cache_path(etf_code, threshold)
                    if cache_path.exists():
                        etf_meta = {
                            'last_updated': datetime.now().isoformat(),
                            'data_count': 0,  # 需要实际读取文件获取
                            'last_date': '',  # 需要实际读取文件获取
                            'data_hash': ''
                        }
                else:
                    # 旧格式
                    etf_meta = threshold_meta.get(etf_code)
    ...
    def save_metadata(self, metadata: MomentumCacheMetadata) -> bool:
        """
        保存缓存元数据 - 使用阈值级别聚合格式
        ...
        try:
            with self._lock:
                ...
                # 转换元数据中的时间戳
                threshold_meta = convert_timestamps(threshold_meta)
    ...
    def load_cache(self, etf_code: str, threshold: str) -> Optional[pd.DataFrame]:
        ...
        try:
            with self._lock:
                ...
                # 验证数据完整性 - 动量振荡器字段
                expected_columns = ['code', 'date', 'momentum_10', 'momentum_20',
                                  'roc_5', 'roc_12', 'roc_25', 'pmo', 'pmo_signal',
                                  'williams_r', 'momentum_strength', 'momentum_acceleration',
                                  'momentum_trend', 'momentum_divergence', 'momentum_volatility',
                                  'calc_time']
                
                # 检查是否有足够的核心字段
                core_columns = ['code', 'date', 'momentum_10', 'roc_5', 'pmo']
    ...
    def save_cache(self, etf_code: str, threshold: str, 
        ...
        try:
            with self._lock:
                ...
                # 创建元数据
                metadata = MomentumCacheMetadata(
                    etf_code=etf_code,
                    threshold=threshold,
                    file_path=str(cache_path),
                    file_size=file_size,
