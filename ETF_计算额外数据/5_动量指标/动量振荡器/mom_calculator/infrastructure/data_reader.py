...
class MomentumDataReader:
    """动量振荡器数据读取器"""
    
    def __init__(self, source_dir: Optional[Path] = None, encoding: str = 'utf-8'):
        """
        初始化数据读取器
        
        Args:
            source_dir: 数据源目录，None则使用配置中的默认路径
            encoding: 文件编码格式
        """
        self.source_dir = source_dir or MomentumConfig.ETF_SOURCE_DIR
        self.encoding = encoding
        self.logger = logging.getLogger(self.__class__.__name__)
        
        # 数据字段映射
        self.field_mapping = MomentumConfig.INPUT_FIELDS
        
        # 必需字段（映射后的英文字段名）
        self.required_fields = ['code', 'date', 'close', 'high', 'low']
        
        # 门槛配置
        self.threshold_config = MomentumConfig.THRESHOLD_CONFIG
        
        self.logger.info(f"数据读取器初始化完成 - 源目录: {self.source_dir}")
    ...
    def read_batch_etf_data(self, etf_codes: Optional[List[str]] = None,
        ...
        try:
            ...
            if all_data:
                ...
                # 最终排序 - 按时间正序，便于动量计算
                combined_df = combined_df.sort_values(['code', 'date'], ascending=[True, True]).reset_index(drop=True)
    ...
    def get_incremental_data(self, etf_code: str, 
        ...
        try:
            ...
            incremental_df = df[df['date'] > last_datetime].copy()
            
            if len(incremental_df) > 0:
                # 转换回字符串格式
                incremental_df['date'] = incremental_df['date'].dt.strftime('%Y-%m-%d')
    ...
    def _apply_threshold_filter(self, df: pd.DataFrame, threshold: str) -> pd.DataFrame:
        ...
        try:
            ...
            if 'amount' in df.columns:
                amount_wan = df['amount'] / 1000  # 千元转万元 (1000千元 = 1万元)
    ...
    def _validate_data_quality(self, df: pd.DataFrame, etf_code: str) -> bool:
        ...
        try:
            ...
            # 检查最少数据量
            min_records = MomentumConfig.MOMENTUM_CONFIG['min_data_points']
