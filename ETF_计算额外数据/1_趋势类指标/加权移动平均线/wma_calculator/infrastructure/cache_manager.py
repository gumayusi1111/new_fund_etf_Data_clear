...
class WMACacheManager:
    """WMA智能缓存管理器 - 参考SMA项目架构"""
    
    def __init__(self, config: Optional[WMAConfig] = None):
        """
        初始化缓存管理器
    ...
    def analyze_etf_changes(self, current_etfs: List[str], threshold: str) -> Dict[str, Set[str]]:
        ...
        analysis = {
            'same_etfs': current_set & cached_set,
            'new_etfs': current_set - cached_set,
            'old_etfs': cached_set - current_set
        }
    ...
    def is_cache_valid_optimized(self, etf_code: str, threshold: str, source_file_path: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                # 检查必要的WMA列是否存在
                required_wma_columns = [f'WMA{period}' for period in (self.config.wma_periods if self.config else [3, 5, 10, 20])]
                wma_columns = [col for col in cache_df.columns if col.startswith('WMA')]
                
                if len(wma_columns) == 0:
                    return False
    ...
    def is_cache_valid(self, etf_code: str, threshold: str, source_file_path: str) -> bool:
        """检查缓存是否有效 - 使用优化版本"""
        if self.config and hasattr(self.config, 'performance_mode'):
    ...
    def _update_global_cache_meta(self) -> None:
        ...
        try:
            ...
            self.save_meta(global_meta, None)  # None表示全局meta
