...
try:
    ...
    from infrastructure.data_reader_optimized import RSIDataReaderOptimized
    from infrastructure.cache_manager import RSICacheManager
    from engines.rsi_engine_optimized import RSIEngineOptimized
    from outputs.csv_handler import RSICSVHandler

except ImportError as e:
    print(f"❌ 导入RSI指标优化模块失败: {str(e)}")
    raise


class RSIMainControllerOptimized:
    """RSI指标优化主业务控制器"""
    ...
    def __init__(self, adj_type="前复权", use_optimized_components=True):
        ...
        try:
            # 初始化配置
            self.config = RSIConfig(adj_type=adj_type)
            ...
            print("✅ RSI指标优化主控制器初始化成功")

        except Exception as e:
            print(f"❌ RSI指标优化主控制器初始化失败: {str(e)}")
            raise
    ...
    def _initialize_optimized_modules(self):
        ...
        try:
            if self.use_optimized_components:
                # 使用优化版本的组件
                self.data_reader = RSIDataReaderOptimized(self.config)
                self.rsi_engine = RSIEngineOptimized(self.config)
                print("🚀 使用优化版本组件")
            else:
                # 这里可以添加原版本组件的回退逻辑
                print("📊 优化组件是唯一选项")
            ...
            # 缓存和输出组件
            self.cache_manager = RSICacheManager(self.config)
            self.csv_handler = RSICSVHandler(self.config)

            print("🔧 RSI指标优化功能模块初始化完成")
    ...
    def calculate_single_etf_optimized(
        ...
        try:
            ...
            print(f"🚀 开始优化计算RSI指标: {etf_code} ({threshold})")
                ...
                if not cached_data.empty:
                    if use_incremental:
                        ...
                        if incremental_result is not None:
                            ...
                            if save_result:
                                self.csv_handler.save_etf_rsi_data(
            ...
            # 使用优化引擎计算RSI指标
            rsi_result = self.rsi_engine.calculate_rsi_indicators_batch(etf_data)

            if rsi_result.empty:
                error_msg = f"RSI指标计算失败: {etf_code}"
                print(f"❌ {error_msg}")
                self.statistics["failed_calculations"] += 1
                self.statistics["failed_etf_list"].append(etf_code)
                return {"success": False, "error": error_msg, "etf_code": etf_code}
            ...
            # 4. 格式化输出数据
            formatted_data = self.rsi_engine.format_output_data(rsi_result, etf_code)
            ...
            self.statistics["successful_calculations"] += 1
            print(
                f"✅ RSI指标优化计算完成: {etf_code} (耗时: {calculation_time:.2f}ms)"
            )
            ...
            return {
                ...
                "data": formatted_data,
                "calculation_time_ms": calculation_time,
                "statistics": self.rsi_engine.calculate_rsi_statistics(rsi_result)
                if hasattr(self.rsi_engine, "calculate_rsi_statistics")
                else {},
            }
    ...
    def _try_incremental_update(self, etf_code, threshold, cached_data, new_etf_data):
        ...
        try:
            ...
            new_etf_data_copy["date"] = new_etf_data_copy["日期"].dt.strftime("%Y-%m-%d")
            ...
            # 使用优化引擎的增量更新功能
            if hasattr(self.rsi_engine, "calculate_incremental_update"):
                # 准备现有数据格式
                ...
                incremental_result = self.rsi_engine.calculate_incremental_update(
                    existing_data_for_calc, truly_new_data
                )

                if not incremental_result.empty:
                    # 格式化增量结果
                    formatted_incremental = self.rsi_engine.format_output_data(
    ...
    def _calculate_incremental_update_rate(self):
        ...
        if self.statistics["processed_etfs"] > 0:
            return (
                self.statistics["incremental_updates"] / self.statistics["processed_etfs"]
            ) * 100
        return 0.0
    ...
    def calculate_batch_etfs(self, etf_codes, threshold):
        """批量计算ETF RSI指标（继承原有逻辑，使用优化计算）"""
        try:
            print(f"🚀 开始优化批量计算RSI指标: {threshold}")
    ...
    def _print_batch_results_optimized(self, batch_results):
        """打印优化的批量处理结果"""
        print(f"\n{'=' * 60}")
        print(f"📊 RSI指标优化批量计算结果 - {batch_results['threshold']}")
        print(f"{'=' * 60}")
        ...
        # 优化效果统计
        avg_time = self.statistics["optimization_metrics"]["average_calculation_time_ms"]
        print("🚀 优化效果:")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 初始化优化控制器
        controller = RSIMainControllerOptimized(use_optimized_components=True)
