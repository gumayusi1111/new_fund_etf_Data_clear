...
class WilliamsConfig:
    ...
    WILLIAMS_PERIODS = {
        'short': 9,      # 短期参数 - 高敏感度
        'standard': 14,  # 标准参数 - 国际通用
        'medium': 21     # 中期参数 - 平滑信号
    }
    
    # 威廉指标衍生参数
    WILLIAMS_DERIVED_PARAMS = {
        'range_period': 5,    # 波动范围计算周期
    ...
    WILLIAMS_THRESHOLDS = {
        'overbought': -20,    # 超买阈值
        'oversold': -80,      # 超卖阈值
        'neutral_upper': -20, # 中性区间上界
        'neutral_lower': -80  # 中性区间下界
    }
    ...
    DATA_QUALITY_REQUIREMENTS = {
        'min_data_points': 21,        # 最少数据点(满足WR21计算)
    ...
    OUTPUT_FIELDS = {
        ...
        'williams_fields': ['wr_9', 'wr_14', 'wr_21'],
        'derived_fields': ['wr_diff_9_21', 'wr_range', 'wr_change_rate'],
    ...
    # 系统性能目标
    PERFORMANCE_TARGETS = {
        'batch_processing_speed': 500,  # ETF/分钟
        'memory_limit_mb': 100,         # 内存限制
        'single_etf_time_ms': 10,       # 单ETF计算时间
    ...
    def _setup_paths(self):
        ...
        # 设置威廉指标工作路径
        williams_root = os.path.dirname(os.path.dirname(self.base_path))
        self.williams_root = williams_root
        
        # 设置缓存和输出路径
        self.cache_base_path = os.path.join(williams_root, "cache")
        self.data_output_path = os.path.join(williams_root, "data")
    ...
    def _setup_system_config(self):
        ...
        self.system_info = {
            ...
            'category': '相对强弱指标',
            'version': '1.0.0',
            'description': '威廉指标(%R)技术分析计算系统',
            'author': 'ETF技术指标计算系统',
            'created_date': datetime.now().strftime('%Y-%m-%d'),
            'supported_indicators': list(self.OUTPUT_FIELDS['williams_fields'] + self.OUTPUT_FIELDS['derived_fields'])
        }
    ...
    def get_williams_periods(self):
        """获取威廉指标周期参数"""
        return self.WILLIAMS_PERIODS
    
    def get_output_fields(self):
        """获取完整输出字段列表"""
        all_fields = []
        all_fields.extend(self.OUTPUT_FIELDS['base_fields'])
        all_fields.extend(self.OUTPUT_FIELDS['williams_fields'])
